        ...
        <div class="row">
            <c:if test="${!empty bookTOList}">
                <table width="100%" class="table table-striped table-bordered table-hover dataTable">
                    ...
                    <tr role="row">
                        <th class="sorting"><a
                                href="${requestScope['javax.servlet.forward.request_uri']}?<c:if test="${!empty action}">action=${action}&</c:if><c:if test="${!empty id}">id=${id}&</c:if><c:if test="${!empty searchRequest}">searchRequest=${searchRequest}&</c:if><c:if test="${!empty currentPage}">page=${currentPage}&</c:if>orderBy=title"><fmt:message
                                key="index.bookList.title"/></a></th>
                        <th class="sorting"><a
                                href="${requestScope['javax.servlet.forward.request_uri']}?<c:if test="${!empty action}">action=${action}&</c:if><c:if test="${!empty id}">id=${id}&</c:if><c:if test="${!empty searchRequest}">searchRequest=${searchRequest}&</c:if><c:if test="${!empty currentPage}">page=${currentPage}&</c:if>orderBy=author"><fmt:message
                                key="index.bookList.author"/></a></th>
                        <th class="sorting"><a
                                href="${requestScope['javax.servlet.forward.request_uri']}?<c:if test="${!empty action}">action=${action}&</c:if><c:if test="${!empty id}">id=${id}&</c:if><c:if test="${!empty searchRequest}">searchRequest=${searchRequest}&</c:if><c:if test="${!empty currentPage}">page=${currentPage}&</c:if>orderBy=publish_year"><fmt:message
                                key="index.bookList.publishYear"/></a></th>
                        <th class="sorting"><a
                                href="${requestScope['javax.servlet.forward.request_uri']}?<c:if test="${!empty action}">action=${action}&</c:if><c:if test="${!empty id}">id=${id}&</c:if><c:if test="${!empty searchRequest}">searchRequest=${searchRequest}&</c:if><c:if test="${!empty currentPage}">page=${currentPage}&</c:if>orderBy=description"><fmt:message
                                key="index.bookList.description"/></a></th>
                        <th class="sorting"><a
                                href="${requestScope['javax.servlet.forward.request_uri']}?<c:if test="${!empty action}">action=${action}&</c:if><c:if test="${!empty id}">id=${id}&</c:if><c:if test="${!empty searchRequest}">searchRequest=${searchRequest}&</c:if><c:if test="${!empty currentPage}">page=${currentPage}&</c:if>orderBy=count"><fmt:message
                                key="index.bookList.count"/></a></th>
                                    ...
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <li class="paginate_button " aria-controls="dataTables-example"
                                                tabindex="0"><a
                                                    href="${requestScope['javax.servlet.forward.request_uri']}?<c:if test="${!empty action}">action=${action}&</c:if><c:if test="${!empty id}">id=${id}&</c:if><c:if test="${!empty searchRequest}">searchRequest=${searchRequest}&</c:if>page=${i}">${i}</a>
                                            </li>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
